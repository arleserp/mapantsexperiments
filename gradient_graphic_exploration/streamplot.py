...
def plot_contour(x, y, z, filename):
    ...
    # show plot
    plt.show()
    plt.savefig("imgs/"+filename+".png", format="png")
...
def plot_data(x, y, z, filename):
    ...
    plt.streamplot(x, y, dffx, dfdy, density=2)
    plt.axis("scaled")
    #plt.show()
    # show plot
    fig = plt.gcf()
    #plt.show()
    fig.savefig("imgs/stream-"+filename+".png", format="png")


def main():

	#args = sys.argv[1:]
    for f in os.listdir("gradient_rep/"):     
