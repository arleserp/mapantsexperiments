...
def load_world_data(filename):
    with open("world_data/"+filename) as f:
        data = f.readlines()
    i = 0
    j = 0
    for line in data:
...
def plot_contour(x, y, z, filename):
    ...
    # show plot
    #plt.show()
    plt.savefig("imgs/contourvec-"+filename+".png", format="png")
...
def plot_data(x, y, z, filename):
    ...
    #plt.quiver(x[5::2,5::2], y[5::2,5::2], dffx[5::2,5::2], dfdy[5::2,5::2], pivot="middle")
    # Adding labels
    ...
    fig = plt.gcf()
    #plt.show()
    fig.savefig("imgs/countour-vec-"+filename+".png", format="png")


def main():

	#args = sys.argv[1:]
    for f in os.listdir("world_data/"):     
        if not f.endswith("ini"):
            print(f"processing: {f}...")
            x, y, z = load_world_data(f)
            plot_data(x, y, z, f)
#plot_contour(x, y, z)
